...
# Function to separate 81-string in subrgoups to mutate some of them 
def mutation(quizz, parent, mutation, number):
    # number: number of selected subgroup for mutation
    
    # Separate string of 81 in subgroups of 9 values
    ...
    # Separate quizz (initial quizz with fixed values and zeros)
    slpit_quizz = split(quizz)
    # Separate parent (string with fixed values and random values)
    ...
    # Define selected subgroups for mutation
    quizz = []
    individual = []
    for i in index:
...
if __name__ == '__main__':
    ...
    print(mutation(quizz, parent, scramble_mutation, 2))